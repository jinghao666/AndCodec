                      ...
                      const enum EANDSysUtilOpenMode mode)
{
  int saved_errno;
  int ret;
  ret = open(p_filename, and_sysutil_translate_openmode(mode)/* | O_NONBLOCK*/);
  if (ret < 0) {
	  saved_errno = errno;
	  and_log_writeline_easy(0, LOG_ERROR, "failed to open file %s, ret %d, %s", 
		  p_filename, saved_errno, strerror(saved_errno));
  }
...
and_sysutil_create_or_open_file(const char* p_filename, unsigned int mode)
{
  int saved_errno;
  int ret;
  ret = open(p_filename, O_CREAT | O_WRONLY/* | O_NONBLOCK*/, mode);
  if (ret < 0) {
	saved_errno = errno;
	and_log_writeline_easy(0, LOG_ERROR, "failed to create file %s, ret %d, %s", 
		p_filename, saved_errno, strerror(saved_errno));
  }
