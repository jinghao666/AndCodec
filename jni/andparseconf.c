...
int and_parseconf_parse(const char* conf_str)
{
	...
	if (!conf_str)
	{
		and_log_writeline_simple(0, LOG_INFO, "null conf string");
		return 0;
	}

	and_log_writeline_easy(0, LOG_INFO, "parse conf: %s", conf_str);
	...
	while (str_getline(&config_conf_str, &config_setting_str, &str_pos))
	{
		...
		if(ret < 0) {
			and_log_writeline_simple(0, LOG_ERROR, "failed to parse conf");
			return -1;
		}
	}
...
and_parseconf_load_setting(const char* p_setting, int errs_fatal)
{
					...
					*p_curr_setting = str_strdup(&s_value_str);
					if (str_equal_text(&s_setting_str, "gop_size")) {
						if ( prosess_gopsize(&s_value_str) < 0) {
							and_log_writeline_simple(0, LOG_ERROR, "failed to set gop_size");						
							return -1;
						}
					}
				}
	...
	if (str_isempty(&s_value_str))
	{
		if (errs_fatal)
		{
			and_log_writeline_easy(0, LOG_ERROR, "missing value in config file for: %s",
	...
	/* Is it a boolean value? */
	{
		...
		while (p_bool_setting->p_setting_name != 0)
		{
			if (str_equal_text(&s_setting_str, p_bool_setting->p_setting_name))
			{
				...
				{
					and_log_writeline_easy(0, LOG_ERROR, "bad bool value in config file for: ",
						str_getbuf(&s_setting_str));
					return -1;
				}
				// should not be here!
	...
	if (errs_fatal)
	{
		...
		and_log_writeline_simple(0, LOG_ERROR, str_getbuf(&info_str));
		str_free(&info_str);
		and_log_writeline_easy(0, LOG_ERROR, "unrecognized variable in config file: ",
			str_getbuf(&s_setting_str));
		return -1;
	}
	//should not be here!
...
int prosess_gopsize(const struct mystr *str_gop)
{
	...
	if (str_get_char_at(str_gop, str_len - 1) == 'x') {
		...
		and_log_writeline_easy(0, LOG_INFO, "gop_size rate mode: gop %d(rate %d)", 
			tunable_keyint_max, ratio);
	}
	else {
		tunable_keyint_max = str_atoi(str_gop);
		and_log_writeline_easy(0, LOG_INFO, "gop_size normal mode: gop %d", 
			tunable_keyint_max);
	}
